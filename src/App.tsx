...
import { LogOut, Shield, LogIn, ArrowRight } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans selection:bg-white/20 selection:text-white overflow-hidden flex flex-col md:flex-row">
      ...
      <AnimatePresence mode="wait">
        {currentView === 'institutional' ? (
          ...
          <motion.div
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="flex-1 flex flex-col items-center justify-center p-6 bg-[#0A0A0A] min-h-screen relative z-10"
          >
            <div className="w-full max-w-5xl bg-[#111111] border border-white/5 rounded-sm p-8 md:p-16 relative overflow-hidden">
              {/* Absolutes for style */}
              <div className="absolute top-0 right-0 p-12 opacity-[0.03]">
                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 underline-offset-8">
                  <div>
                    <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-white/40 mb-4 block">Operações Sincronizadas</span>
                    <h1 className="text-4xl md:text-6xl font-light uppercase">
                      Painel <span className="font-semibold uppercase ml-2">{userAuthData.nome.split(' ')[0]}</span>
                    </h1>
                    {userAuthData.allFields && (
                      <div className="mt-4 flex flex-wrap gap-2">
                         {userAuthData.allFields['Empresa'] && (
                           <span className="text-[10px] px-2 py-1 bg-white/5 border border-white/10 text-white/50 font-bold uppercase tracking-widest">{userAuthData.allFields['Empresa']}</span>
                         )}
                         {userAuthData.allFields['Cargo'] && (
                           <span className="text-[10px] px-2 py-1 bg-amber-500/10 border border-amber-500/20 text-amber-500 font-bold uppercase tracking-widest">{userAuthData.allFields['Cargo']}</span>
                         )}
                         {userAuthData.allFields['Superintendência'] && (
                           <span className="text-[10px] px-2 py-1 bg-blue-500/10 border border-blue-500/20 text-blue-500 font-bold uppercase tracking-widest">{userAuthData.allFields['Superintendência']}</span>
                         )}
                      </div>
                    )}
                  </div>
                  <button 
                    ...
                    className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-bold text-white/30 hover:text-white transition-colors py-3 px-6 border border-white/10 rounded-sm bg-white/5 active:scale-95"
                  >
                    <LogOut size={14} />
                    Finalizar Sessão
                  </button>
            ...
            <div className="mt-12 flex gap-12 border-t border-white/5 pt-8 w-full max-w-5xl justify-between">
               <div className="flex gap-8">
                 <span className="text-[9px] uppercase tracking-[0.2em] text-white/20 hover:text-white/40 transition-colors cursor-pointer">Segurança Ativa</span>
                 <span className="text-[9px] uppercase tracking-[0.2em] text-white/20 hover:text-white/40 transition-colors cursor-pointer">{firebaseUser.email}</span>
               </div>
               <span className="text-[9px] uppercase tracking-[0.2em] text-white/10 font-mono">ID: {firebaseUser.uid.slice(0, 8)}...</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
